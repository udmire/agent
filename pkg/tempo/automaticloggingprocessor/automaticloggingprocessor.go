		...
		for j := 0; j < ilsLen; j++ {
			...
			for k := 0; k < spanLen; k++ {
				...
				if p.cfg.Spans {
					...
					p.exportToLoki(typeSpan, traceID, p.spanLabels(keyValues), keyValues...)
				}

				if p.cfg.Roots && span.ParentSpanID().IsEmpty() {
					keyValues := append(p.spanKeyVals(span), p.processKeyVals(rs.Resource(), svc)...)
					p.exportToLoki(typeRoot, traceID, p.spanLabels(keyValues), keyValues...)
				}

				if p.cfg.Processes && lastTraceID != traceID {
					lastTraceID = traceID
					keyValues := p.processKeyVals(rs.Resource(), svc)
					p.exportToLoki(typeProcess, traceID, p.spanLabels(keyValues), keyValues...)
				}
			}
		}
	}
...
func (p *automaticLoggingProcessor) exportToLoki(kind string, traceID string, labels model.LabelSet, keyvals ...interface{}) {
	if p.done.Load() {
		return
	}
